...
.note{background:#fff7e6;border:1px solid #f1d9a5;border-radius:8px;padding:8px 12px;margin:0 0 16px;font-size:14px;color:var(--ink)}@media (prefers-color-scheme:dark){.note{background:#2e2612;border-color:#5c4a1a}}
.foot{font-size:13px;margin-top:20px}code{font-size:13px;background:var(--soft);padding:1px 5px;border-radius:4px}.err{background:var(--errbg);border:1px solid var(--errline);border-radius:8px;padding:10px;margin-bottom:12px}</style></head>
<body><main><h1>PatchOwner</h1><p class="note">⚠️ @@BANNER@@</p><p>Upload your technology list. See exactly which exploited vulnerabilities would have reached whom, and which ones stayed quiet.</p>
@@ERROR@@
